...
from app import db, c_abort, newswirepostfile
...
from app.newswire_resources.newswire_post_file_library.schemas import (
    NewswirePostLibraryFileSchema, NewswirePostLibraryFileReadArgsSchema)

from queueapp.thumbnail_tasks import convert_file_into_thumbnail


class NewswirePostLibraryFileAPI(AuthResource):
    """
    Create, update, delete API for newswire post library files
    """

    @swag_from('swagger_docs/newswire_post_file_library_post.yml')
    def post(self):
        """
        Create a file
        """
        newswire_post_library_file_schema = NewswirePostLibraryFileSchema()
        json_data = {}
        try:
            # validate and deserialize input into object
            data, errors = newswire_post_library_file_schema.load(json_data)
            if errors:
        ...
        if 'filename' in request.files:
            ...
            fpath, fname, ferrors, ftype = store_file(
                newswirepostfile, request.files['filename'],
        ...
        try:
            ...
            if file_type_for_thumbnail(fname):
                convert_file_into_thumbnail.s(
                    True, data.row_id, BASE.MOD_NEWSWIRE_POST_FILE_LIBRARY,
                    fpath).delay()
            else:
    ...
    @swag_from('swagger_docs/newswire_post_file_library_delete.yml')
    def delete(self, row_id):
        ...
        try:
            # first find model
            model = NewswirePostLibraryFile.query.get(row_id)
    ...
    @swag_from('swagger_docs/newswire_post_file_library_get.yml')
    def get(self, row_id):
        """
        Get a file by id
        """
        ...
        try:
            # first find model
            model = NewswirePostLibraryFile.query.options(joinedload(
                NewswirePostLibraryFile.account)).get(row_id)
            if model is None or model.deleted:
                c_abort(404, message='File id: %s does not exist' %
                                     str(row_id))
            # check ownership
            if model.created_by != g.current_user['row_id']:
                abort(403)
            result = newswire_post_library_file_schema.dump(model)
        except Forbidden as e:
            raise e
        except HTTPException as e:
            raise e
...
class NewswirePostLibraryFileListAPI(AuthResource):
    """
    Read API for library file lists, i.e, more than 1 library file
    """
    model_class = NewswirePostLibraryFile

    def __init__(self, *args, **kwargs):
        kwargs['special_fields'] = ['filename_url', 'linked_contacts']
        super(NewswirePostLibraryFileListAPI, self).__init__(*args, **kwargs)
    ...
    def build_query(self, filters, pfields, sort, pagination, query_session,
        ...
        query_filters['base'].append(
            NewswirePostLibraryFile.created_by == g.current_user['row_id'])
        query = self._build_final_query(query_filters, query_session, operator)

        query = query.options(joinedload(NewswirePostLibraryFile.account))

        return query, db_projection, s_projection, order, paging

    @swag_from('swagger_docs/newswire_post_file_library_get_list.yml')
    def get(self):
        """
        Get the list
        """
        newswire_post_library_file_read_schema = \
            NewswirePostLibraryFileReadArgsSchema(strict=True)
        models = []
        total = 0
        ...
        try:
            ...
            query, db_projection, s_projection, order, paging =\
                self.build_query(filters, pfields, sort, pagination,
                                 db.session.query(NewswirePostLibraryFile),
                                 operator)
            ...
            if s_projection:
                ...
                newswire_post_library_file_schema =\
                    NewswirePostLibraryFileSchema(only=s_projection)
            # make query
            ...
            result = newswire_post_library_file_schema.dump(models, many=True)
        except HTTPException as e:
            raise e
        except Exception as e:
            current_app.logger.exception(e)
