"""
Models for "newswire post file library" package.
"""
...
class NewswirePostLibraryFile(BaseModel):

    __tablename__ = 'newswire_post_library_file'
    # config key for root folder of files
    root_folder_key = 'NEWSWIRE_POST_FILE_FOLDER'

    account_id = db.Column(db.BigInteger, db.ForeignKey(
        'account.id', name='newswire_post_library_file_account_id_fkey',
        ondelete='CASCADE'), nullable=False)
    created_by = db.Column(db.BigInteger, db.ForeignKey(
        'user.id', name='newswire_post_library_file_created_by_fkey',
        ondelete='CASCADE'), nullable=False)
    updated_by = db.Column(db.BigInteger, db.ForeignKey(
        'user.id', name='newswire_post_library_file_updated_by_fkey',
        ondelete='CASCADE'), nullable=False)
    ...
    account = db.relationship('Account', backref=db.backref(
        'newswire_postfiles', lazy='dynamic'))
    creator = db.relationship(
        'User', backref=db.backref('newswire_postfiles', lazy='dynamic'),
        foreign_keys='NewswirePostLibraryFile.created_by')
    ...
    def __init__(self, created_by=None, updated_by=None, *args, **kwargs):
        ...
        super(NewswirePostLibraryFile, self).__init__(*args, **kwargs)

    def __repr__(self):
        return '<NewswirePostLibraryFile %r>' % (self.row_id)

    def load_urls(self, with_redirect=False, expires_in=3600,
                  thumbnail_only=False):
        """
        Populates the file_url and thumbnail_url dynamic properties
        """
        s3_url = ''
