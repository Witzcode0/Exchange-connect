...
class CRMLibraryFile(BaseModel):

    __tablename__ = 'crm_library_file'
    # config key for root folder of files
    root_folder_key = 'CRM_LIBRARY_FILE_FOLDER'
    ...
    updated_by = db.Column(db.BigInteger, db.ForeignKey(
        'user.id', name='crm_library_file_updated_by_fkey',
        ondelete='CASCADE'), nullable=False)
    user_id = db.Column(db.BigInteger, db.ForeignKey(
        'user.id', name='crm_library_file_user_id_fkey', ondelete='CASCADE'))
    ...
    account = db.relationship('Account', backref=db.backref(
        'crmlibraryfiles', lazy='dynamic'))
    creator = db.relationship(
        'User', backref=db.backref('crmlibraryfiles', lazy='dynamic'),
        foreign_keys='CRMLibraryFile.created_by')
    ...
    def __init__(self, created_by=None, updated_by=None, *args, **kwargs):
        ...
        super(CRMLibraryFile, self).__init__(*args, **kwargs)

    def __repr__(self):
        return '<CRMLibraryFile %r>' % (self.row_id)
