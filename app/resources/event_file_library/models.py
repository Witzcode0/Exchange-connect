"""
Models for "event file library" package.
"""
...
class EventLibraryFile(BaseModel):

    __tablename__ = 'event_library_file'
    # config key for root folder of files
    root_folder_key = 'EVENT_FILE_FOLDER'

    account_id = db.Column(db.BigInteger, db.ForeignKey(
        'account.id', name='event_library_file_account_id_fkey',
        ondelete='CASCADE'), nullable=False)
    created_by = db.Column(db.BigInteger, db.ForeignKey(
        'user.id', name='event_library_file_created_by_fkey',
        ondelete='CASCADE'), nullable=False)
    updated_by = db.Column(db.BigInteger, db.ForeignKey(
        'user.id', name='event_library_file_updated_by_fkey',
        ondelete='CASCADE'), nullable=False)
    ...
    account = db.relationship('Account', backref=db.backref(
        'eventfiles', lazy='dynamic'))
    creator = db.relationship(
        'User', backref=db.backref('eventfiles', lazy='dynamic'),
        foreign_keys='EventLibraryFile.created_by')
    ...
    def __init__(self, created_by=None, updated_by=None, *args, **kwargs):
        ...
        super(EventLibraryFile, self).__init__(*args, **kwargs)

    def __repr__(self):
        return '<EventLibraryFile %r>' % (self.row_id)
