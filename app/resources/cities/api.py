...
from app.resources.cities.schemas import (
    CitySchema, CityReadArgsSchema)
from app.base import constants as APP
from app.resources.roles.models import ROLE


class CityAPI(AuthResource):
    """
    ...
    def post(self):
        ...
        try:
            ...
            data, errors = city_schema.load(json_data)
            if errors:
        ...
        return {'message': 'City added: %s' % str(data.row_id),
        ...
        try:
            ...
            if model is None or model.deleted:
                c_abort(404, message='City id: %s does not exist' %
                                     str(row_id))
            result = CitySchema(
                exclude=CitySchema._default_exclude_fields).dump(model)
        except Forbidden as e:
            raise e
        except HTTPException as e:
            raise e
...
class CityListAPI(BaseResource):
    """
    ...
    def __init__(self, *args, **kwargs):
        super(CityListAPI, self).__init__(*args, **kwargs)
    ...
    def get(self):
        ...
        Get the list
        """
        city_read_schema = CityReadArgsSchema(strict=True)
        models = []
        total = 0
        ...
        try:
            ...
            query, db_projection, s_projection, order, paging =\
                self.build_query(filters, pfields, sort, pagination,
                                 db.session.query(City), operator)
            # making a copy of the main output schema
            city_schema = CitySchema(
                exclude=CitySchema._default_exclude_fields)
            if db_projection:
                # change the query to include only requested fields
                query = query.options(load_only(*db_projection))
            if s_projection:
                # change the schema to include only requested fields
                city_schema = CitySchema(only=s_projection)
            # make query
            ...
            result = city_schema.dump(models, many=True)
        except HTTPException as e:
            raise e
        except Exception as e:
            current_app.logger.exception(e)
