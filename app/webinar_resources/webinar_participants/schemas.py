"""
Schemas for "webinar participants" related models
"""
...
from app.webinar_resources.webinar_participants import constants as \
    WEBINARPARTICIPANT


class WebinarParticipantSchema(ma.ModelSchema):
    """
    Schema for loading "webinar participant" from request,
    ...
    participant_email = field_for(
        WebinarParticipant, 'participant_email', field_class=fields.Email,
        validate=[validate.Length(
            max=WEBINARPARTICIPANT.PARTICIPANT_EMAIL_MAX_LENGTH,
            error=APP.MSG_LENGTH_EXCEEDS)])
    participant_first_name = field_for(
        WebinarParticipant, 'participant_first_name', validate=[
            validate.Length(max=WEBINARPARTICIPANT.PARTICIPANT_NAME_MAX_LENGTH,
                            error=APP.MSG_LENGTH_EXCEEDS)])
    participant_last_name = field_for(
        WebinarParticipant, 'participant_last_name', validate=[
            validate.Length(max=WEBINARPARTICIPANT.PARTICIPANT_NAME_MAX_LENGTH,
                            error=APP.MSG_LENGTH_EXCEEDS)])
    participant_designation = field_for(
        WebinarParticipant, 'participant_designation', validate=[
            validate.Length(
                max=WEBINARPARTICIPANT.PARTICIPANT_DESIGNATION_MAX_LENGTH,
                error=APP.MSG_LENGTH_EXCEEDS)])
    sequence_id = field_for(
        WebinarParticipant, 'sequence_id', validate=validate.Range(
    ...
    links = ma.Hyperlinks({
        'self': ma.URLFor(
            'webinar_api.webinarparticipantapi', row_id='<row_id>'),
        'collection': ma.URLFor('webinar_api.webinarparticipantlistapi')
    }, dump_only=True)
    ...
    def validate_participant(self, data):
        ...
        if ('participant_id' not in data and 'participant_email' not in data):
            error_args = [
                APP.MSG_NON_EMPTY, 'participant_id, participant_email']

        if ('participant_id' in data and 'participant_email' in data):
            error_args = [
                'Both participant_id and participant_email' +
                ' should not be there', 'participant_id, participant_email']
...
class WebinarParticipantReadArgsSchema(BaseReadArgsSchema):
    """
    Schema for reading "webinar participant" filters from request args
    """
