"""
Schemas for "corporate access event participants" related models
"""
...
class CorporateAccessEventParticipantSchema(ma.ModelSchema):
    """
    Schema for loading "corporate access event participant" from request,
    and also formatting output
    """
    sequence_id = field_for(
        CorporateAccessEventParticipant, 'sequence_id',
        validate=validate.Range(
            min=APP.SEQUENCE_ID_MIN_VALUE,
            error=APP.SEQUENCE_ID_MIN_VALUE_ERROR))

    class Meta:
        model = CorporateAccessEventParticipant
        include_fk = True
    ...
    links = ma.Hyperlinks({
        'self': ma.URLFor(
            'corporate_access_api.corporateaccesseventparticipantapi',
            row_id='<row_id>'),
        'collection': ma.URLFor(
            'corporate_access_api.corporateaccesseventparticipantlistapi')
    }, dump_only=True)
    ...
    def validate_participant_user(self, data, original_data):
        ...
        same account of CAEvent creator
        """
        error = False
        user_data = None
        if ('participant_id' in original_data and
            ...
            user_data = User.query.filter(and_(
                User.row_id == original_data['participant_id'], or_(
                    ...
                    User.account_id == g.current_user['account']['row_id']))
            ).options(
...
class CorporateAccessEventParticipantReadArgsSchema(BaseReadArgsSchema):
    """
    Schema for reading "corporate access event participant"
