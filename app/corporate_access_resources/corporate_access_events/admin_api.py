...
from app import (
    ...
    caeventattachmentfile, caeventtranscriptfile, caeventaudiofile,
    flaskapp)
...
from app.corporate_access_resources.corporate_access_events.schemas import (
    ...
    CorporateAccessEventEditSchema, AdminCorporateAccessEventSchema)
...
class AdminCorporateAccessEventAPI(AuthResource):
    """
    ...
    def post(self):
        ...
        """
        corporate_access_event_schema = AdminCorporateAccessEventSchema()
        ...
        try:
            ...
            if data.is_draft is False:
                # if audio and transcript file uploaded then announcement
                # create
                event_sub_type = CARefEventSubType.query.filter_by(
                    row_id=data.event_sub_type_id).first()
                if (not data.is_draft and event_sub_type.show_time
                        and data.account.account_type == ACCOUNT.ACCT_CORPORATE
                        and (data.audio_filename or data.transcript_filename)):
                    if data.audio_filename:
                        if not CorporateAnnouncement.query.filter(
                            ...
                            db.session.add(CorporateAnnouncement(
                                announcement_date=data.started_at,
                                category=CORPO.CANNC_CONCAL_TRANSCRIPTS,
                                subject=data.title,
                                description=data.description,
                                ca_event_audio_file_id=data.row_id,
                    ...
                    if data.transcript_filename:
                        if not CorporateAnnouncement.query.filter(
                                ...
                                ca_event_transcript_file_id == data.row_id
                                ).first():
                            db.session.add(CorporateAnnouncement(
                                announcement_date=data.started_at,
                                category=CORPO.CANNC_CONCAL_TRANSCRIPTS,
    ...
    def put(self, row_id):
            ...
            if 'audio_filename' in request.form:
                ...
                if (request.form['audio_filename'] ==
                    ...
                    if audio['delete']:
                        ...
                        CorporateAnnouncement.query.filter_by(
                            ca_event_audio_file_id=row_id).delete()
                ...
                if (request.form['transcript_filename'] ==
                    ...
                    if transcript['delete']:
                        ...
                        if ferrors:
                            return ferrors['message'], ferrors['code']
                        CorporateAnnouncement.query.filter_by(
                            ca_event_transcript_file_id=row_id).delete()
        ...
        try:
            ...
            if collaborator_data and not collaborator_editor:
                ...
                db.session.commit()
            # if audio and transcript file uploaded then announcement create
            event_sub_type = CARefEventSubType.query.filter_by(
                row_id=data.event_sub_type_id).first()
            if (not data.is_draft and event_sub_type.show_time
                    and data.account.account_type == ACCOUNT.ACCT_CORPORATE
                ...
                if data.audio_filename:
                    if not CorporateAnnouncement.query.filter(
                        ...
                        db.session.add(CorporateAnnouncement(
                            announcement_date=data.started_at,
                            category=CORPO.CANNC_CONCAL_TRANSCRIPTS,
                            subject=data.title, description=data.description,
                            ca_event_audio_file_id=row_id,
                            account_id=data.account_id,
                            created_by=data.created_by,
                            updated_by=data.updated_by))
                if data.transcript_filename:
                    if not CorporateAnnouncement.query.filter(
                            CorporateAnnouncement.ca_event_transcript_file_id ==
                            row_id).first():
                        db.session.add(CorporateAnnouncement(
                            announcement_date=data.started_at,
                            category=CORPO.CANNC_CONCAL_TRANSCRIPTS,
                            subject=data.title, description=data.description,
                            ca_event_transcript_file_id=row_id,
                            account_id=data.account_id,
                            created_by=data.created_by,
                            updated_by=data.updated_by))
                db.session.commit()
...
class AdminCorporateAccessEventListAPI(AuthResource):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(AdminCorporateAccessEventListAPI, self).__init__(*args, **kwargs)
...
class AdminCorporateAccessEventCancelledAPI(AuthResource):
    """
    ...
    def put(self, row_id):
        ...
        try:
            ...
            if not model.cancelled:
                ...
                db.session.commit()
                CorporateAnnouncement.query.filter(or_(
                    CorporateAnnouncement.ca_event_audio_file_id == row_id,
                    CorporateAnnouncement.ca_event_transcript_file_id ==
                    row_id)).update({CorporateAnnouncement.deleted: True},
                ...
                if ('USE_ELASTIC_SEARCH' in flaskapp.config
                        and flaskapp.config['USE_ELASTIC_SEARCH']):
                    add_to_index(APP.DF_ES_INDEX, model)
        except HTTPException as e:
            raise e
        except Exception as e:
            db.session.rollback()
